...
<html>
    <head>
     <link rel="stylesheet" href="<?php echo base_url("css/createbus_css.css") ?>" type="text/css" />
        <title>Cancellation Sms updation</title>
<style type="text/css">
table.gridtable {
	...
	<script src="<?php echo base_url("js/jquery.min.js");?>" type="text/javascript"></script>
        <script>
            function ChangeData(){
				...
				//alert(op);
                $.post('view_api_can_sms_update',{op:op},function(res){
				//alert(res);
                 $('#sp').html(res); 
                 //$('#tbl').hide(); 
                });
        ...
        <h4 align='center'>Cancellation Sms updation</h4>
...
foreach($query as $row){
	...
	echo "<td style='font-size:12px';>".$row->operator_title."</td>";
        echo "<td style='font-size:12px';>".$row->id."</td>";
	...
	echo "<td class='space'><span style='cursor:pointer; font-weight:bold; font-size:12px; text-decoration:underline;'>".anchor('master_control/edit_apisms?uid='.$uid.'&is_api_sms='.$status, 'Update', '')."</span></td>";
