       ...
       <style>
        ...
        <script>
            function showRefundForm(j) {
                ...
                $.post('show_ser_RefundFormdata', {j: j,paid:paid,ccharge:ccharge,camt:camt,refamt:refamt}, function(res) {
                    //alert(res);
        ...
        //alert(k);
         ...
         //alert(k);
                ...
                var pnr1 = $('#pnr'+ k).val();
                    ...
                    $.post('do_service_amountRefund', {amt: amt1, ser: ser1, tra: tra1, ticket: ticket1, agid:agid1, pnr:pnr1}, function(res) {
                        alert(res);
                        if (res == 1)
                        {
                            alert("refunded");
...
else
{
    ...
    foreach ($query1->result() as $value)
	{
		...
		if ($status == 'service cancelled' || $status == 'Service cancelled')
		{
                   $status = 'Refund';
		}
       ...
       <td align="center" style="font-size:12px;width="60px" id="ref_st"> ';
        if($status == 'Refund')
	   {
