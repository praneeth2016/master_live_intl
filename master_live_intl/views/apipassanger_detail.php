...
foreach ($query1->result() as $value) {
    ...
    if ($res->num_rows() > 0) {
        ...
        $tkt_no = $value->tkt_no;
        $pnr = $value->pnr;
        $service_no = $value->service_num;
        $bdate = $value->blocked_time;
        $jdate = $value->journey_date;
        $source = $value->from_name;
        $dest = $value->to_name;
