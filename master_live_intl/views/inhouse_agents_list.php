	...
	<script src="<?php echo base_url("js/jquery.min.js");?>" type="text/javascript"></script>
        <script>
            function ChangeData(){
                ...
                $.post('getBranch_db',{op:op},function(res){
                 $('#sp').html(res); 
                 $('#tbl').hide(); 
                });
            }
        ...
        if(s===1)
          ...
          if(con===true)
           {
            $.post("update_status",{status:s,id:uid},function(res){
                 if(res==1)
                     {
                         alert('status has been changed');
                         setTimeout('window.location = "inhouse_agents"');
                     }
                 else{
        ...
        <h4 align='center'>Branch Agents List</h4>
