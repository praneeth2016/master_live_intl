	...
	<script src="<?php echo base_url("js/jquery.min.js");?>" type="text/javascript"></script>
        <script>
            function ChangeData(){
                ...
                $.post('getPostpaid_db',{op:op},function(res){
                 $('#sp').html(res); 
                 $('#tbl').hide(); 
                });
            }
        ...
        if(s===1)
          ...
          if(con===true)
           {
            $.post("status_change_postpaid",{status:s,id:uid},function(res){
                 if(res==1)
                     {
                         alert('status has been changed');
                         setTimeout('window.location = "postpaid_agents"');
                     }
                 else{
		  ...
		  <h4 align="center" >Postpaid Agents List</h4>
